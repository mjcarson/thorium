...
//! caller**. The version comes out of the file's own header.
...
use crate::cart::codec::{Decode, MAX_DECODE_CHUNK, MAX_INPUT_CHUNK, OutBuf, new_decoder};
...
///
/// This holds no pinned state of its own, so it is [`Unpin`] whenever the reader is, and it is
