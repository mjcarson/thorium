...
//!   decrypts the optional header and then yields garbage for the body.
...
use crate::cart::codec::{
    Decode, Encode, MAX_DECODE_CHUNK, OutBuf, SOFT_OUTPUT_CAP, TailWithholder,
};
...
impl Decode for V1Decoder {
    ...
    fn finish(&mut self, _out: &mut OutBuf) -> Result<(), Error> {
        ...
        let body_end = match self.state {
            ...
            _ => self.body_end.ok_or(Error::Truncated {
...
#[cfg(test)]
mod tests {
    use super::{V1Decoder, V1Encoder, V1Options};
    ...
    fn our_output_has_the_layout_we_claim() {
        ...
        assert_eq!(header.version, super::CartVersion::V1);
