...
use super::{CartVersion, MAX_BLOCK_SIZE_LOG2, MIN_BLOCK_SIZE_LOG2, V2Options};
use crate::cart::codec::{Decode, Encode, OutBuf, SOFT_OUTPUT_CAP};
    ...
    fn parse(raw: &[u8]) -> Result<Self, Error> {
        ...
        let raw: &[u8; OPT_HEADER_LEN] = raw
            ...
            .ok_or(Error::Truncated {
                section: Section::OptionalHeader,
            })?;
...
impl Decode for V2Decoder {
    ...
    fn finish(&mut self, _out: &mut OutBuf) -> Result<(), Error> {
        match self.state {
            ...
            V2State::Length | V2State::Block { .. } => Err(Error::Truncated {
                section: Section::Body,
            }),
            V2State::TrailerLength | V2State::Trailer { .. } => Err(Error::Truncated {
...
mod tests {
    ...
    use crate::cart::codec::{Decode, Encode, OutBuf};
    use crate::cart::versions::{CartVersion, V2Options};
