...
impl SigmaRuleContext {
    ...
    pub async fn new(thorium: &Arc<Thorium>) -> Result<Self, Error> {
...
impl From<&Entity> for MaybeEntity {
    /// Wrap an existing entity in a maybe entity
    ///
    /// # Arguments
    ///
...
impl SigmaActionCache {
    ...
    pub fn add_entity(
        ...
        match self.pending_entities.entry(hash) {
            ...
            Entry::Vacant(entry) => {
                // update our stats for this entity kind
                stats.by_entity[req.kind()].created += 1;
            ...
            if let Some(root_kind) = entity.root_kind() {
                ...
                let root = match entities.first() {
                    ...
                    None => {
                        ...
                        let msg = "Missing root {root_kind} from {entity:#?}";
...
impl EntityMap {
    ...
    pub async fn populate(
        ...
        for req in entities {
            ...
            // track that we got one of these entities
            local_stats.by_entity[req_kind].scanned += 1;
...
impl SigmaRuleWorker {
    ...
    async fn create_flag(
        ...
        action_cache.add_association(rule, AssociationKind::CreatedBy, &rule.original, flag_index);
        Ok(index)
    }
    ...
    pub async fn scan_event(&self, event: Event) -> Result<SigmaRuleStats, Error> {
        ...
        for applies_to in &event_data.applies_to {
            ...
            for index in &self.context.applies_to[entity_kind] {
                // get this rule
                let rule = self.context.combined.get(*index).unwrap();
